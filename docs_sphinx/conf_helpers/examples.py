...
# Doxygen markup that shows up inside `@example` description text: a `@ref`
# cross-reference (with an optional quoted label) and Markdown image embeds.
# The generated example page is NOT run through the `_translate` source-read
# pipeline (only tutorial/api docs are — see translate._source_read), so these
# would otherwise render as literal `@ref …` text and a broken bare-filename
# image. Resolve them here at generation time using the same shared anchor /
# image indexes translate itself uses.
...
def _resolve_brief_markup(brief: str) -> str:
    """Resolve `@ref` links and bare-filename image embeds in an `@example`
    description to plain Markdown so MyST renders a working link + the
    screenshot instead of literal source text.

    `@ref` resolution mirrors translate's `_ref_repl`: an enabled-module
    anchor becomes an internal `/docname` link; otherwise the Doxygen tagfile
    supplies an external URL (so refs into not-yet-onboarded modules — e.g.
    the imgproc tutorial — still link out to docs.opencv.org); failing both,
    a bare in-page `#anchor`. Bare image filenames resolve through the global
    `_IMAGE_INDEX` basename map (Doxygen's flat IMAGE_PATH equivalent)."""
    ...
    def _img(m: "re.Match") -> str:
        ...
        if not hit:
            return m.group(0)
        # End the line at the image with a Markdown hard break (`\`) so any
        # following prose — e.g. "Check the corresponding tutorial …" — drops
        # to the next line, matching how the @example description is authored
        # (image on its own line, explanation on the next).
        return f'![{alt}](/{hit})\\\n'
...
def _generate_example_pages(examples_dir: pathlib.Path) -> None:
    ...
    for display, source in _EXAMPLE_PAGES_NEEDED.items():
        ...
        if brief:
            ...
            # docs.opencv.org puts its description. Emitted as a MyST
            # paragraph (class attached via attrs_block) rather than a raw
            # `<p>` so an embedded `@ref` link / screenshot image actually
            # renders; a raw HTML block would make MyST skip the markup.
