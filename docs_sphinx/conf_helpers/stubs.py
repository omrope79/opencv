...
def _group_by_section_header(items: list[dict]) -> list[tuple[str, list[dict]]]:
    """Split a member list into contiguous runs sharing the same Doxygen
    ``@name`` section header (the member dict's ``section_header``; ``""`` =
    the default header-less sectiondef). Order is preserved, so each run maps
    to one summary table optionally preceded by its header sub-heading."""
...
def _render_member_detail(m: dict, full_name: str) -> list[str]:
    ...
    # Row 2: template clause (if any) + declaration, clubbed into ONE paragraph
    # via a Markdown hard break (`\`) — template on its own line, declaration on
    # the next, no separator between. Emitted as inline code (NOT a ```cpp fence)
    # so the token-linkifier makes type names clickable and so the typedef
    # declaration isn't wrapped in code-block chrome.
    _sig = ([f"`{tmpl}`"] if tmpl else []) + [f"`{decl}`"]
    out += ["\\\n".join(_sig), ""]

    # `#include <…>` line (from the member's <location file>), shown as its own
    # card row — matches the api/core_basic Function Documentation blocks and
    # docs.opencv.org. Applies to typedefs / functions / variables alike.
    inc = (m.get("include_file") or "").strip()
    if inc:
        # `{.opencv-api-include}` (attrs_block) marks this paragraph so CSS can
        # render it as a distinct inset box within the card, like Doxygen.
...
def _render_core_basic_func(m: dict, idx: int, total: int,
    ...
    # Row 2: template clause (if any) + signature, clubbed into ONE paragraph
    # via a Markdown hard break (`\`) so they sit on consecutive lines with no
    # separator. Inline code keeps the token-linkifier (step 8g) active.
