...
def _parse_member_sections(cd) -> dict[str, list[dict]]:
    ...
    for sd in cd.findall("sectiondef"):
        # Doxygen `@name <header>` member-group title (e.g. "Shorter aliases
        # for the most popular specializations of Vec<T,n>"). Lives on the
        # <sectiondef kind="user-defined">; carried onto each member so the
        # stub writers can render the grouping sub-heading above the table.
