...
# Image basename index (mirrors Doxygen's flat IMAGE_PATH lookup). Built BEFORE
# the API-stub step below because `_generate_api_stubs` -> `_generate_example_pages`
# resolves bare-filename image embeds in `@example` briefs through this map
# (e.g. Smoothing.cpp's screenshot); populating it afterwards left those images
# unresolved and rendering broken.
